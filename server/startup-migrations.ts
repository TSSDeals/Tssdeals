...
export const STARTUP_MIGRATIONS: readonly VersionedMigration<StartupContext>[] = [
  ...
  {
    ...STARTUP_MIGRATION_MANIFEST[11],
    async up(context) {
      const statements = [
        `CREATE TABLE IF NOT EXISTS deal_inbox_media (
        ...
        `CREATE INDEX IF NOT EXISTS deal_inbox_media_created_at_idx
          ON deal_inbox_media(created_at)`,
      ];
      for (const statement of statements) await context.execute(sql.raw(statement));
    },
  },
  {
    ...STARTUP_MIGRATION_MANIFEST[12],
    async up(context) {
      const statements = [
        `CREATE TABLE IF NOT EXISTS deal_inbox_media (
          id VARCHAR(40) PRIMARY KEY,
  ...
  {
    ...STARTUP_MIGRATION_MANIFEST[17],
    async up(context) {
      const statements = [
        `ALTER TABLE deals ADD COLUMN IF NOT EXISTS classification_locked BOOLEAN NOT NULL DEFAULT false`,
        `ALTER TABLE deals ADD COLUMN IF NOT EXISTS classification_updated_at TIMESTAMP`,
      ];
      for (const statement of statements) await context.execute(sql.raw(statement));
    },
  },
