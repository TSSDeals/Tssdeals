...
} from "../server/protective-equipment-history";
...
const outputPath = resolve(
  valueAfter("--output") ?? "./protective-equipment-history-preview.json",
);
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: ProtectiveAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalProtectiveEquipment);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    if (!row
        ...
        || !proposal.proposedSportId
        || !proposal.proposedCanonicalEquipmentTypeId) {
      return [];
    }
    return [{
      id: row.id,
