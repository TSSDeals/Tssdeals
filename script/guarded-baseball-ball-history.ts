...
} from "../server/baseball-ball-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./baseball-ball-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: BaseballBallAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalBaseballBall);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "baseball", equipmentTypeId: "bb-balls" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'baseball', equipment_type_id = 'bb-balls'
          WHERE id = $1
            AND title = $2
