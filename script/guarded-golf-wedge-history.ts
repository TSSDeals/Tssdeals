...
} from "../server/golf-wedge-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./golf-wedge-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: GolfWedgeAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalGolfWedge);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "golf", equipmentTypeId: "golf-wedges" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'golf', equipment_type_id = 'golf-wedges'
          WHERE id = $1
            AND title = $2
