...
} from "../server/baseball-training-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./baseball-training-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: BaseballTrainingAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalBaseballTraining);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "baseball", equipmentTypeId: "bb-training" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'baseball', equipment_type_id = 'bb-training'
          WHERE id = $1
            AND title = $2
