...
} from "../server/softball-bat-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./softball-bat-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: SoftballBatAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalSoftballBat);
