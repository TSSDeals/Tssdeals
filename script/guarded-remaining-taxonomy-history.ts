...
} from "../server/remaining-taxonomy-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./remaining-taxonomy-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: RemainingTaxonomyAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections.flatMap((proposal) => {
  const destination = approvedRemainingDestination(proposal);
  return destination ? [{ proposal, destination }] : [];
});
...
try {
  ...
  const ids = approved.map(({ proposal }) => proposal.dealId);
  ...
  const changes = approved.flatMap(({ proposal, destination }) => {
