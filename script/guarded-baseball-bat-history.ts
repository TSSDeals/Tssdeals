...
} from "../server/baseball-bat-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./baseball-bat-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: BatAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalBaseballBat);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "baseball", equipmentTypeId: "bb-bats" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'baseball', equipment_type_id = 'bb-bats'
          WHERE id = $1
            AND title = $2
