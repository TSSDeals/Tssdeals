...
} from "../server/golf-driver-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./golf-driver-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: GolfDriverAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalGolfDriver);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "golf", equipmentTypeId: "golf-drivers" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'golf', equipment_type_id = 'golf-drivers'
          WHERE id = $1
            AND title = $2
