...
} from "../server/fastpitch-glove-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./fastpitch-glove-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: FastpitchGloveAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalFastpitchGlove);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "fastpitch-softball", equipmentTypeId: "fp-gloves" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'fastpitch-softball', equipment_type_id = 'fp-gloves'
          WHERE id = $1
            AND title = $2
