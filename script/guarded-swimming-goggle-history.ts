...
} from "../server/swimming-goggle-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./swimming-goggle-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: SwimmingGoggleAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalSwimmingGoggle);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "swimming", equipmentTypeId: "swim-goggles" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'swimming', equipment_type_id = 'swim-goggles'
          WHERE id = $1
            AND title = $2
