...
} from "../server/baseball-glove-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./baseball-glove-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: GloveAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalBaseballGlove);
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'baseball', equipment_type_id = 'bb-gloves'
          WHERE id = $1
            AND title = $2
