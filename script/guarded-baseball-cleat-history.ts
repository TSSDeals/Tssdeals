...
} from "../server/baseball-cleat-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./baseball-cleat-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: BaseballCleatAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalBaseballCleat);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "baseball", equipmentTypeId: "bb-cleats" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'baseball', equipment_type_id = 'bb-cleats'
          WHERE id = $1
            AND title = $2
