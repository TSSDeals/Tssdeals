...
} from "../server/golf-putter-history";
...
const outputPath = resolve(valueAfter("--output") ?? "./golf-putter-history-preview.json");
...
const audit = JSON.parse(await readFile(resolve(packetPath), "utf8")) as {
  reviewPacket: { proposedCorrections: GolfPutterAuditProposal[] };
};
const approved = audit.reviewPacket.proposedCorrections
  .filter(isApprovedHistoricalGolfPutter);
...
try {
  ...
  const changes = approved.flatMap((proposal) => {
    ...
    return [{
      ...
      after: { sportId: "golf", equipmentTypeId: "golf-putters" },
    }];
  });
  const log = {
    version: 1,
  ...
  if (apply) {
    for (const change of changes) {
      const result = await client.query(
        `UPDATE deals
            SET sport_id = 'golf', equipment_type_id = 'golf-putters'
          WHERE id = $1
            AND title = $2
